...
let headerHTML = '<ul> <li><a href="https://gokuchan.space/">home</a></li>' + 
...
const postsJsonPath = relativePath + "/posts.json";

fetch(postsJsonPath)
  .then(function (res) {
    if (!res.ok) {
      throw new Error("Failed to load posts list: " + postsJsonPath);
    }
    return res.json();
  })
  .then(function (postsJson) {
    postsArray = postsJson.map(function (p) {
      return [p];
    });
